...
import { fetchProducts } from '../services/api'; // Import the updated api.js
...
const MarketPage = () => {
  ...
  const fetchAllProducts = async () => {
    ...
    if (response.success) {
      setProducts(response.data.data); // Assuming API returns { success: true, data: { data: [...] } }
  ...
  const handleToggleFavorite = (item) => {
    if (favorites.includes(item._id)) {
      ...
      setFavorites(favorites.filter((favId) => favId !== item._id));
    } else {
      // Add to favorites
  ...
  return (
    <View style={[styles.container, { backgroundColor: currentTheme.backgroundColor }]}>
      ...
      <View style={styles.header}>
        ...
        {/* Uncomment if you want to include the cart button in the header
        <TouchableOpacity
